...
import { messages, conversations, conversationMembers, messageReads } from '../../db/schema/index.js';
...
export class MessagingService {
  async createConversation(
    ...
    const id = generateId();
    await db.insert(conversations).values({ id, type, name: name ?? null, createdBy });
    ...
    await db.insert(conversationMembers).values(
      allMembers.map((userId) => ({
        conversationId: id,
        userId,
        role: userId === createdBy ? 'owner' as const : 'member' as const,
      })),
    );
  ...
  async getUserConversations(userId: string): Promise<Conversation[]> {
    ...
    return memberships.map((m) => m.conversation) as any[];
  }
  ...
  async editMessage(messageId: string, senderId: string, content: string): Promise<void> {
    await db
      .update(messages)
  ...
  async markDelivered(messageId: string, userId: string): Promise<void> {
    await db
      .insert(messageReads)
      .values({ messageId, userId, deliveredAt: new Date().toISOString() })
      .onConflictDoNothing();
  }
  ...
  async markRead(messageId: string, userId: string): Promise<void> {
    await db
      ...
      .where(and(eq(messageReads.messageId, messageId), eq(messageReads.userId, userId)));
  }
}
