...
export class CallsService {
  async initiateCall(
    ...
    const id = generateId();
    await db.insert(calls).values({ id, conversationId, type, initiatedBy });
    await db.insert(callParticipants).values({ callId: id, userId: initiatedBy });
  ...
  async updateStatus(id: string, status: Call['status']): Promise<void> {
    ...
    if (status === 'ended' || status === 'missed' || status === 'declined') updates.endedAt = now;
  ...
  async addParticipant(callId: string, userId: string): Promise<void> {
    await db.insert(callParticipants).values({ callId, userId }).onConflictDoNothing();
  }

  async removeParticipant(callId: string, userId: string): Promise<void> {
    const now = new Date().toISOString();
    await db.update(callParticipants).set({ leftAt: now }).where(
  ...
  async getHistory(userId: string): Promise<Call[]> {
    ...
    return participations.map((p) => p.call) as any[];
  }
}
