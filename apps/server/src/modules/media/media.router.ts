...
const MIME_TYPE_MAP: Record<string, 'image' | 'video' | 'voice'> = {
  ...
  'audio/ogg': 'voice', 'audio/mpeg': 'voice', 'audio/webm': 'voice', 'audio/mp4': 'voice',
};
...
export async function mediaRouter(fastify: FastifyInstance, { io }: { io: TypedIO }): Promise<void> {
  ...
  fastify.post('/upload', async (request, reply) => {
    ...
    const meta = uploadMetaSchema.parse(Object.fromEntries(
      Object.entries(data.fields).map(([k, v]) => [k, (v as any).value]),
    ));
    ...
    const attachment = await mediaService.processAndStore(buffer, mimeType, mediaType, message.id);
